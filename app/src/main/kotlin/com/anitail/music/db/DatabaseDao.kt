...
import com.anitail.music.db.entities.AlbumWithSongs
...
interface DatabaseDao {
    ...
    @Transaction
    @Query("SELECT * FROM song WHERE isEpisode = 1 AND (mediaStoreUri IS NOT NULL OR downloadUri IS NOT NULL) ORDER BY dateDownload DESC")
    fun downloadedPodcastEpisodesByCreateDateAsc(): Flow<List<Song>>

    @Transaction
    @Query("SELECT * FROM song WHERE isEpisode = 1 AND (mediaStoreUri IS NOT NULL OR downloadUri IS NOT NULL) ORDER BY title")
    fun downloadedPodcastEpisodesByNameAsc(): Flow<List<Song>>

    @Transaction
    @Query("SELECT * FROM song WHERE isEpisode = 1 AND (mediaStoreUri IS NOT NULL OR downloadUri IS NOT NULL) ORDER BY totalPlayTime")
    ...
    @Query("""
        ...
        AND (SELECT COUNT(1) FROM song_artist_map JOIN song ON song_artist_map.songId = song.id WHERE artistId = artist.id AND song.inLibrary IS NOT NULL AND song.isEpisode = 1) > 0
        ORDER BY artist.name COLLATE NOCASE ASC
    """)
