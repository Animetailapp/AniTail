 ...
 *
 * Modified for anitail to use manual SharedPreferences control instead of automatic scaling.
...
class DensityScaler : BaseLifecycleContentProvider() {
    ...
    companion object {
        private const val PREFS_NAME = "anitail_settings"
        ...
        private fun getScaleFactorFromPreferences(context: Context): Float {
            return try {
                ...
                android.util.Log.w(
                    "DensityScaler",
                    "Failed to read scale factor from preferences",
                    e
                )
